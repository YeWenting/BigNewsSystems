...
###2. 最少时间策略
与*最小费用策略*不同在于，时间这一维在交通方式选择中起到了影响。我们先考虑一个简化问题：**不考虑必经城市**，在 time = k 时从城市 i 最早能在什么时间到城市 j （等待时间加转移时间）。这是一个三维的问题，首先由于加了时间，我们不能够使用简单的 Floyd 算法求解。我们考虑用 SPFA（Shortest Path Faster Algorithm）还是Dijkstra
